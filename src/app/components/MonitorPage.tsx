...
export function MonitorPage({ assignmentId, onNavigate }: MonitorPageProps) {
  ...
  const fetchJobStatus = useCallback(async (jobId: string) => {
    try {
      ...
      setJobState({
        ...
        currentStage: response.progress.stage,
        progress: response.progress.blocksCompleted / response.progress.totalBlocks * 100,
        currentWordCount: 0,
        targetWordCount: 3000,
        errorMessage: response.error,
        createdAt: new Date().toISOString(),
        startedAt: response.startedAt?.toString(),
      });
      setIsPaused(false);
      setIsLoading(false);
      return response;
  ...
  useEffect(() => {
    if (assignment) {
      // Tokens info would be available from assignment data
      setTokensUsed(0);
    }
  }, [assignment?.id]);
